...
class MaintenanceRateService {
  ...
  public getByProject(projectId: number): MaintenanceRate[] {
    return dbService.query<MaintenanceRate>('SELECT * FROM maintenance_rates WHERE project_id = ? ORDER BY financial_year DESC', [projectId]);
  }
  ...
  public update(id: number, rate: Partial<MaintenanceRate>): boolean {
    const fields = Object.keys(rate)
      .filter(key => key !== 'id' && key !== 'project_name' && key !== 'created_at')
      .map(key => `${key} = ?`)
      .join(', ');
    const values = Object.keys(rate)
      .filter(key => key !== 'id' && key !== 'project_name' && key !== 'created_at')
      .map(key => rate[key as keyof MaintenanceRate]);

    const result = dbService.run(`UPDATE maintenance_rates SET ${fields} WHERE id = ?`, [...values, id]);
  ...
  public getSlabs(rateId: number): MaintenanceSlab[] {
    return dbService.query<MaintenanceSlab>('SELECT * FROM maintenance_slabs WHERE rate_id = ? ORDER BY due_date ASC', [rateId]);
  }
